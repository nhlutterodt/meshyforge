...
| Rust test isolation | 11 of 197 Rust `--lib` tests failed reproducibly on one developer machine, all asserting "no API key configured" | `AppState::new()` called the real-keychain free function `security::get_key()` directly instead of going through the already-existing `Keychain` trait seam, so every test building an `AppState` (including `commands/api.rs`'s own `make_no_key_state()`) silently observed a real credential left in the machine's OS keychain from earlier manual verification | Added `AppState::new_with_keychain(dir, &dyn Keychain)`; `AppState::new()` now delegates to it with `RealKeychain`, and every test constructor uses it with `InMemoryKeychain::new()` instead |
...
## 11. A Test Seam That Exists Isn't a Test Seam Until Every Caller Uses It

### Observation

`cargo test --lib` failed 11 of 197 tests, reproducibly, on one developer's machine. Every failure asserted some form of "no API key is configured" — `state.provider().is_none()`, `result.is_err()` with code `MISSING_API_KEY` — and got the opposite: a real provider, or a real HTTP 404 from an actual network call. The same 11 tests passed cleanly in CI and on a fresh clone.

### Root Cause

`commands/keychain.rs` already had the right shape: `set_api_key_with_keychain_inner`, `get_api_key_inner`, and `delete_api_key_with_keychain_inner` all take a `&dyn Keychain` parameter, so their tests inject `InMemoryKeychain` and never touch the real OS credential store. `security::Keychain` (the trait) and `security::InMemoryKeychain` (the mock) were built specifically for this.

`AppState::new()` was the one call site that never adopted the pattern. It called the free function `security::get_key()` — which is hardcoded to `RealKeychain.get()` — directly, with no way to override it. Every test helper that built an `AppState` (`app_state.rs`'s own tests, `commands/api.rs::make_no_key_state()`/`make_test_state()`, `commands/assets.rs::make_state()`, `commands/keychain.rs::make_state()`) called `AppState::new(dir)`, so all of them silently inherited whatever key `RealKeychain` found on the machine running the tests — instead of the empty state their own comments and assertions promised. Lesson 8 (`docs/LESSONS_LEARNED.md#8`) had previously stored a real key in this exact machine's Windows Credential Manager (`meshyforge` / `meshy_api_key`) while proving `RealKeychain` actually persists — confirmed present via `cmdkey /list`, target `meshy_api_key.meshyforge`. That leftover credential is what the tests were silently reading. CI never had this problem only because no CI runner has ever had that credential stored.

### Resolution

Added `AppState::new_with_keychain(data_dir, keychain: &dyn Keychain)`, which does the real construction work; `AppState::new()` is now a one-line wrapper calling it with `&RealKeychain`, preserving production behavior exactly. Every test constructor across `app_state.rs`, `commands/api.rs`, `commands/assets.rs`, and `commands/keychain.rs` now calls `AppState::new_with_keychain(dir, &InMemoryKeychain::new())` instead of `AppState::new(dir)`. `cargo test --lib` went from 186/197 to 197/197 passing, with zero test logic changed — only how each test constructs its `AppState`.

### Guardrails

- A trait built "for testing" (per its own doc comment) only provides isolation at the call sites that actually take it as a parameter. Before trusting a mock/injection seam, grep every constructor of the type it's meant to isolate (`AppState::new(` in this case) and confirm none of them bypass it through a free function or hardcoded default.
- A test whose assertion depends on "nothing is configured" (`is_none()`, a specific "missing X" error code) is implicitly asserting something about the *environment*, not just the code under test — it will pass or fail based on ambient machine state unless every path to that state is constructed through an injectable seam.
- A test suite that passes in CI but fails reproducibly on one developer machine (not flaky — reproducible) is evidence of environment leakage, not environment flakiness. Check what that machine has that CI doesn't (a stored credential, a config file, an env var) before assuming a race condition or nondeterminism.
